...
check("block header names the count", r1.output.startsWith(`[nana:knowledge] ${r1.hits.length} pointer`));
...
check("a second stale prompt does not spawn again (lock)", ensureFreshIndex(Date.now(), () => spawned++) === "locked" && spawned === 1);
fs.rmSync(path.join(home, "build.lock"), { force: true });
